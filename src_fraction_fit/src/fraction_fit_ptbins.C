    ...
    const char* output = nullptr)      // 如果为nullptr，则自动生成文件名
{
    ...
    for(size_t icent = 0; icent < cent_bins.size(); ++icent) {
        ...
        for(int ipt=0; ipt<nPtBins; ++ipt){
            ...
            // 检查数据质量
            if(data_integral < 100) {
                std::cout << "Warning: Very low data statistics (integral < 100)" << std::endl;
            }
            if(data_integral < 10) {
                std::cout << "Error: Extremely low data statistics (integral < 10) - skipping this bin" << std::endl;
                continue;
            }

            // 确保数据直方图没有负值
            for(int i=1; i<=h_data_proj->GetNbinsX(); i++) {
                if(h_data_proj->GetBinContent(i) < 0) {
                    h_data_proj->SetBinContent(i, 0);
                }
            }

            std::cout << "Final data histogram bins: " << h_data_proj->GetNbinsX() << std::endl;
            ...
            for(int itmp=0; itmp<nTemplates; ++itmp){
                ...
                // //
                // if(h_proj->GetNbinsX() != h_data_proj->GetNbinsX()) {
                //     std::cerr << "Error: Bin mismatch! Data: " << h_data_proj->GetNbinsX()
                //               << ", MC: " << h_proj->GetNbinsX() << std::endl;
                //     continue;
                // }
